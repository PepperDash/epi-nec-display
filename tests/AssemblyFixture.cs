...
public static class AssemblyFixture
{
    ...
    private static MetadataLoadContext CreateContext()
    {
        ...
        // runtime), so the plugin's own deps.json omits every dependency entirely. project.assets.json
        // has the full pre-exclusion dependency graph and is present after any restore/build.
    ...
    // like "Foo" does not match "class FooBar".
